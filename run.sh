...
if [[ -f $SWAGGER_JSON ]]; then
  sed -i "s|http://petstore.swagger.io/v2/swagger.json|swagger/swagger.json|g" index.html
  sed -i "s|http://example.com/api|swagger/swagger.json|g" index.html
else
...
exec http-server -a 0.0.0.0 -p $PORT $*
